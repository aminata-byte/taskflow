...
class AdminController extends Controller
{
    public function index()
    {
        ...
        $totalProjects = Project::where('user_id', $adminId)->count();
        $totalTasks    = Task::whereHas('column.project', fn($q) => $q->where('user_id', $adminId))->count();
        $totalTeams    = Team::count();

        // Tâches par statut (uniquement projets de l'admin)
        $tasksDone = Task::whereHas('column', fn($q) => $q->where('name', 'Terminé'))
            ->whereHas('column.project', fn($q) => $q->where('user_id', $adminId))
            ->count();
        ...
        // Tâches en retard (uniquement projets de l'admin)
        ...
        $members = User::where('role', 'user')
            ...
            ->map(function ($user) {
                ...
                $user->tasks_late       = $user->assignedTasks->filter(
                    fn($t) => $t->column?->name !== 'Terminé' && $t->due_date && Carbon::parse($t->due_date)->isPast()
                )->count();
                return $user;
            });

        // Projets de l'admin avec progression
        $projects = Project::where('user_id', $adminId)
            ->with(['columns.tasks'])
            ->get()
            ->map(function ($project) {
                ...
                $late     = $project->columns->where('name', '!=', 'Terminé')->flatMap->tasks->filter(
                    fn($t) => $t->due_date && Carbon::parse($t->due_date)->isPast()
                )->count();
