    ...
    <div class="page-container">
        ...
        <div class="page-header">
            <div>
                ...
                <p class="page-subtitle"> {{ $team->project->title }} • Vos tâches assignées</p>
            </div>
        ...
        <div class="card" style="margin-bottom:2rem;">
            ...
            <div style="display:grid; grid-template-columns: repeat({{ $columns->count() }}, 1fr); gap:1rem;">
                @foreach ($columns as $column)
                    <div class="kanban-column" data-column-id="{{ $column->id }}" ondragover="event.preventDefault()"
                        ...
                        <div class="column-header">
                            ...
                            <span
                                class="column-count">{{ isset($myTasks[$column->id]) ? $myTasks[$column->id]->count() : 0 }}</span>
                        </div>

                        <div class="tasks-list" id="column-{{ $column->id }}">
                            @if (isset($myTasks[$column->id]))
                                @foreach ($myTasks[$column->id] as $task)
                                    <div class="task-card" draggable="true" data-task-id="{{ $task->id }}"
                                        ...
                                        <div
                                            ...
                                            @if ($task->priority)
                                                <span
                                                    class="priority-badge priority-{{ $task->priority }}">{{ $task->priority }}</span>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
        ...
        @if (!empty($teamTasks))
            <div class="card">
                <h2 style="font-family:'Sora',sans-serif; font-size:1.1rem; font-weight:700; margin-bottom:1.5rem;">
                    👥 Tâches de l'équipe (lecture seule)
                </h2>

                @foreach ($teamTasks as $memberId => $data)
                                ...
                                <div
                                    ...
                                    @if (isset($data['tasks'][$column->id]))
                                        @foreach ($data['tasks'][$column->id] as $task)
                                            <div
                                                style="background:var(--bg-card); border-radius:8px; padding:10px; margin-bottom:8px; border:1px solid var(--border); opacity:0.8;">
                                                <div style="font-size:0.875rem; font-weight:600;">{{ $task->title }}</div>
                                                @if ($task->priority)
                                                    <span class="priority-badge priority-{{ $task->priority }}"
                                                        style="margin-top:6px; display:inline-block;">{{ $task->priority }}</span>
                                                @endif
                                            </div>
                                        @endforeach
                                    @else
    ...
    <script>
        ...
        function dropTask(event, columnId) {
            ...
            fetch('{{ route('member.move-task') }}', {
                ...
                .then(data => {
                    if (data.success) {
                        // Mettre a jour le compteur des colonnes
                        updateCounts();
                    }
                })
                .catch(() => {
